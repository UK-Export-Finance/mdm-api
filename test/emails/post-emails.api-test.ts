...
describe('POST /emails', () => {
  ...
  const { mdmPath, request, postEmailsResponse } = new PostEmailsGenerator(valueGenerator).generate({
    numberToGenerate: 1,
  });
  ...
  it('calls NotifyClient.sendEmail', async () => {
    ...
    expect(sendEmailMethodMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        reference: expect.stringMatching(new RegExp(`^${escapeStringRegexp(request[0].templateId)}-\\d*$`)),
      }),
    );
  });
    ...
    it.each([
      {
        ...
        request: [{ ...request, sendToEmailAddress: 'test@example' }],
        expectedError: 'sendToEmailAddress must be an email',
      },
      {
        desc: 'Email format is not valid2',
        request: [{ ...request, sendToEmailAddress: 'testexample.com' }],
        expectedError: 'sendToEmailAddress must be an email',
      },
      {
        desc: 'Email is required',
        request: (() => {
          const [{ sendToEmailAddress: _sendToEmailAddress, ...all_rest }] = request;
          return [all_rest];
        })(),
  ...
  describe('field validation', () => {
    ...
    it('returns a 200 response if optional field personalisation is not present', async () => {
      const [{ personalisation: _personalisation, ...requestWithoutPersonalisation }] = request;
