...
describe('OdsService - findBusinessCentre', () => {
  ...
  it('should call odsStoredProcedureService.call', async () => {
    // Act
    await service.findBusinessCentre(EXAMPLES.BUSINESS_CENTRE.CODE);
  ...
  it('should return a mapped business centre', async () => {
    // Act
    const result = await service.findBusinessCentre(EXAMPLES.BUSINESS_CENTRE.CODE);

    // Assert
    const { results } = JSON.parse(mockStoredProcedureOutput);
    const [jsonResult] = results;

    const expected = mapBusinessCentre(jsonResult);
  ...
  describe('when a business centre is not found', () => {
    it('should throw an error', async () => {
      ...
      const expected = new Error(`No business centre ${EXAMPLES.BUSINESS_CENTRE.CODE} found in ODS`);
